...
然后你得写一个 `UserDaoImpl`，每个方法都是一段 JdbcTemplate 调用。增删改查，每个实体都这么来一遍。代码量翻倍，但做的事都差不多——改一个字段名，N 个地方跟着改。你不是在写业务逻辑，你是在写"数据访问层的体力活"。

Spring Data JPA 的思路：**把这些重复的 CRUD 操作抽象成接口，框架自动生成实现。你连 Impl 类都不用写。**
...
这里有个容易踩的坑：`@GeneratedValue` 的策略选择。MySQL 用 `IDENTITY`（数据库自增主键），PostgreSQL 用 `SEQUENCE`（序列），别选错了。选错了不会报编译错误，但运行时会出各种诡异问题。
...
JpaRepository 自带的 CRUD 方法不够用怎么办？比如你想"根据名字查用户"。Spring Data JPA 提供了一种很酷的方式：**在接口里按规则定义方法名，框架自动解析成查询语句。**
...
方法名查询有个明显的问题：**条件一多，方法名就爆炸**。比如"根据名字或邮箱查询，年龄在 18-60 之间，状态为活跃，按创建时间倒序"——你难道要写 `findByNameOrEmailAndAgeBetweenAndStatusOrderByCreateTimeDesc`？
...
## 审计字段：createTime / updateTime 不用手动设

几乎每张表都有 `create_time` 和 `update_time`。每次新增、修改都手动 set 一下？漏了怎么办？测试环境数据乱了，十有八九是某个地方忘了设时间。
...
**一个常见问题**：审计字段不生效。大概率是你用了 `@Query` 写的更新语句——它绕过了实体生命周期，不会触发审计事件。解决方法是先查出来再 save：
...
**我的观点**：审计字段自动填充是非常值得开启的功能。让它自动来，少一个出错的机会。
